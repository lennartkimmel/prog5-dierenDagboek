    ...
    @if(auth()->user()->likes->count() > 2) 
        <div class="container">
            <div class="justify-content-center">
                <div><strong>{{ $posts->count()}} </strong> posts geplaatst</div>
                ...
                @foreach($posts as $post)
                    <div class="col-4 pb-4 mt-1 mr-1 border border-dark">
                        <a href="/post/{{ $post->id }}" class="font-weight-bold text-dark">
                            <div class="pt-1 font-weight-bold h5">{{$post->caption}}</div>
                        </a>
                        <a href="/post/{{ $post->id }}" class="font-weight-bold text-muted">
                            <div class="pb-2 font-weight-bold h6">{{$post->tags->name}}</div> 
                        </a>
                        <a href="/post/{{ $post->id }}">
